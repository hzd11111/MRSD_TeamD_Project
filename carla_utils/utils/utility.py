...
class PathPlan(object):
    __slots__ = ['tracking_pose', 'future_poses', 'tracking_speed', 'reset_sim', 
                'path_planner_terminate', 'end_of_action', 'action_progress']

    def __init__(self, tracking_pose = Pose2D(), future_poses = [], tracking_speed = 0., 
                 reset_sim = False, path_planner_terminate = False, end_of_action = False, action_progress = 0.): 
...
class LaneStatus(object):
    ...
    def __init__(self, lane_vehicles = [], lane_points = [], lane_id = 0, crossing_pedestrain = []): 
    ...
    @classmethod
    def fromRosMsg(cls, msg): 
        cls.lane_vehicles = msg.lane_vehicles #[Vehicle.fromRosMsg(v) for v in msg.lane_vehicles]
        cls.lane_points = msg.lane_points #[LanePoint.fromRosMsg(lp) for lp in msg.lane_points]
        cls.lane_id = msg.lane_id
        cls.crossing_pedestrain = msg.crossing_pedestrain #[Pedestrain.fromRosMsg(ped) for ped in msg.crossing_pedestrain]
        return cls

    def toRosMsg(self):
        msg = LaneStatusMsg()
        msg.lane_vehicles = self.lane_vehicles #[v.toRosMsg() for v in self.lane_vehicles]
        msg.lane_points = self.lane_points  #[l.toRosMsg() for l in self.lane_points]
        msg.lane_id = self.lane_id
        msg.crossing_pedestrain = self.crossing_pedestrain #[p.toRosMsg() for p in self.crossing_pedestrain]
        return msg
...
class ParallelLane(LaneStatus):
    ...
    def __init__(self, lane_vehicles = [], lane_points = [], lane_id = 0, crossing_pedestrain = [],
                 same_direction = False, left_to_the_current = False, next_lane = False, lane_distance = 0.): 
        super(ParallelLane, self).__init__(lane_vehicles, lane_points, lane_id, crossing_pedestrain)
...
class PerpendicularLane(LaneStatus):
    ...
    def __init__(self, lane_vehicles = [], lane_points = [], lane_id = 0, crossing_pedestrain = [],
                 intersecting_distance = 0., directed_right = False): 
        super(PerpendicularLane, self).__init__(lane_vehicles, lane_points, lane_id, crossing_pedestrain)
...
class EnvDesc(object):
    __slots__ = ['cur_vehicle_state', 'current_lane', 'next_intersection', 'adjacent_lanes', 
                'speed_limit', 'reward_info', 'pedestrain', 'global_path']

    def __init__(self, cur_vehicle_state = None, current_lane = CurrentLane(), next_intersection = [], 
                 adjacent_lanes = [], speed_limit = 0., reward_info = None, pedestrain = [], global_path = GlobalPath()): 
    ...
    @classmethod
    def fromRosMsg(cls, msg): 
        cls.cur_vehicle_state = msg.cur_vehicle_state #Vechile.fromRosMsg(msg.cur_vehicle_state)
        cls.current_lane = CurrentLane.fromRosMsg(msg.current_lane)
        cls.next_intersection = [PerpendicularLane.fromRosMsg(pline) for pline in msg.next_intersection]
        cls.adjacent_lanes = [ParallelLane.fromRosMsg(pline) for pline in msg.adjacent_lanes]
