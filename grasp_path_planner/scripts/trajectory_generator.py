...
import copy
from settings import TARGET_SPEED


class TrajGenerator:
    SAME_POSE_THRESHOLD = 2
    SAME_POSE_LOWER_THRESHOLD = 0.01
    MAX_GLOBAL_SPEED = TARGET_SPEED
    MAX_STRAIGHT_SPEED = TARGET_SPEED
    ...
    def stop(self, sim_data):
        ...
        for point_ind, path_point in enumerate(global_path_points):
            ...
            if (
                single_pose.isInfrontOf(curr_vehicle_global_pose)
                and single_pose.distance(curr_vehicle_global_pose)
                > TrajGenerator.SAME_POSE_LOWER_THRESHOLD
                and single_pose.distance(curr_vehicle_global_pose) < 1.5
            ):
        ...
        if False:
            ...
            distance = curr_vehicle_global_pose.distance(
                global_path_points[-1].global_pose
            )
    ...
    def globalConstSpeedTraj(self, sim_data):
        ...
        if not self.current_action == RLDecision.GLOBAL_PATH_CONSTANT_SPEED:
            self.reset(
                RLDecision.GLOBAL_PATH_CONSTANT_SPEED,
                sim_data.cur_vehicle_state.speed,
                sim_data.reward_info.time_elapsed,
            )
        ...
        action_progress = (
            new_sim_time - self.action_start_time
        ) / self.traj_parameters["action_duration"]
        ...
        if self.global_path_pointer >= len(global_path_points):
            ...
            print(
                "Lane Navigation Terminated",
                self.global_path_pointer,
                "/",
                len(global_path_points),
            )

        new_path_plan = PathPlan()
        new_path_plan.tracking_pose = tracking_pose
        new_path_plan.reset_sim = False
        new_path_plan.tracking_speed = min(
            max(self.traj_parameters["min_speed"], self.start_speed),
            self.MAX_GLOBAL_SPEED,
        )
    ...
    def globalAccelerateTraj(self, sim_data):
        ...
        if not self.current_action == RLDecision.GLOBAL_PATH_ACCELERATE:
            self.reset(
                RLDecision.GLOBAL_PATH_ACCELERATE,
                sim_data.cur_vehicle_state.speed,
                sim_data.reward_info.time_elapsed,
            )
        ...
        new_path_plan.tracking_speed = min(
            max(
                self.traj_parameters["min_speed"],
                self.start_speed
                + action_progress * self.traj_parameters["accelerate_amt"],
            ),
            self.MAX_GLOBAL_SPEED,
        )
        new_path_plan.end_of_action = end_of_action
    ...
    def globalDecelerateTraj(self, sim_data):
        ...
        if not self.current_action == RLDecision.GLOBAL_PATH_DECELERATE:
            self.reset(
                RLDecision.GLOBAL_PATH_DECELERATE,
                sim_data.cur_vehicle_state.speed,
                sim_data.reward_info.time_elapsed,
            )
        ...
        new_path_plan.tracking_speed = min(
            max(
                self.traj_parameters["min_speed"],
                self.start_speed
                - action_progress * self.traj_parameters["decelerate_amt"],
            ),
            self.MAX_GLOBAL_SPEED,
        )
        if new_path_plan.tracking_speed < self.LOW_SPEED_THRESHOLD:
    ...
    def tempConstSpeed(self, sim_data):
        ...
        new_path_plan.reset_sim = False
        new_path_plan.tracking_speed = (
            max(self.traj_parameters["min_speed"], self.start_speed) + 1
        )
        new_path_plan.end_of_action = True
        new_path_plan.action_progress = 1.0
    ...
    def constSpeedTraj(self, sim_data):
        ...
        new_path_plan.tracking_pose = current_lane.frenetToGlobal(next_point_frenet)
        new_path_plan.reset_sim = False
        new_path_plan.tracking_speed = max(
            self.traj_parameters["min_speed"], self.start_speed
        )
        if new_path_plan.tracking_speed < self.LOW_SPEED_THRESHOLD:
            new_path_plan.tracking_speed = 0.0
        new_path_plan.end_of_action = end_of_action
    ...
    def accelerateTraj(self, sim_data):
        ...
        tracking_pose_frenet = next_point_frenet
        acc_per_sec = (
            self.traj_parameters["accelerate_amt"]
            / self.traj_parameters["action_duration"]
        )
        for ts in np.arange(
            self.traj_parameters["action_time_disc"],
    ...
    def decelerateTraj(self, sim_data):
        ...
        new_path_plan.tracking_speed = max(
            self.traj_parameters["min_speed"],
            self.start_speed
            - action_progress * (self.traj_parameters["decelerate_amt"] + 0),
        )
        if new_path_plan.tracking_speed < self.LOW_SPEED_THRESHOLD:
            new_path_plan.tracking_speed = 0.0
        new_path_plan.end_of_action = end_of_action
        ...
        tracking_pose_frenet = next_point_frenet
        dec_per_sec = (
            self.traj_parameters["decelerate_amt"]
            / self.traj_parameters["action_duration"]
        )
        for ts in np.arange(
            self.traj_parameters["action_time_disc"],
    ...
    def laneChangeTraj(self, sim_data, rl_decision):
        ...
        if not self.current_action == rl_decision:
            ...
            if rl_decision == RLDecision.SWITCH_LANE_LEFT:
                for parallel_lane in sim_data.adjacent_lanes:
                    if (
                        parallel_lane.adjacent_lane
                        and parallel_lane.left_to_the_current
                        and parallel_lane.same_direction
                    ):
                ...
                for parallel_lane in sim_data.adjacent_lanes:
                    if (
                        parallel_lane.adjacent_lane
                        and not parallel_lane.left_to_the_current
                        and parallel_lane.same_direction
                    ):
                        target_lane = parallel_lane
                        break
            ...
            neutral_spline_left = self.cubicSplineGen(
                lane_distance, sim_data.cur_vehicle_state.speed
            )
        ...
        while self.path_pointer < len(self.generated_path):
            ...
            if (
                pose_speed.isInfrontOf(curr_vehicle_global_pose)
                and pose_speed.distance(curr_vehicle_global_pose)
                > TrajGenerator.SAME_POSE_LOWER_THRESHOLD
            ):
                break
        ...
        if end_of_action:
            ...
            new_path_plan.tracking_pose.theta = self.generated_path[
                self.path_pointer
            ].theta
            new_path_plan.tracking_speed = min(
                self.generated_path[self.path_pointer].speed, self.MAX_STRAIGHT_SPEED
            )
        new_path_plan.reset_sim = False
        new_path_plan.end_of_action = end_of_action
        new_path_plan.action_progress = action_progress
