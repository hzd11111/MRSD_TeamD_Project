...
class StateManager:
    ...
    def createLaneChangeState(self, env_desc):
        ...
        laneChangeState["adjacent"] = np.array(adj_lane_vehicles_concatenated)
        ...
        laneChangeState["front"] = np.array([ego_vehicle_state + front_vehicle_state])
        laneChangeState["back"] = np.array([ego_vehicle_state + back_vehicle_state])
        ...
        laneChangeState["pedestrians"] = np.array(pedestrian_states_concatenated)
        # Lane Distance
        # TODO: Identify which lane to consider for lane distance
        laneChangeState["lane_distance"] = 0
