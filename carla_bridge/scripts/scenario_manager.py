...
class CustomScenario:
    def __init__(self, client, carla_handler) -> None:
        ...
        self.scenarios_town05 = [
            [[37], -2, -3, 320, 20]
        ]  # [[[21,22],-1,-2,0,10]], [[37], -2, -3, 0, 0]]
    ...
    def reset(
        self, warm_start=False, warm_start_duration=5
        ...
        if False:
            (
                ...
                self.curr_lane_id,
                self.ego_spawn_idx,
                self.vehicle_init_speed,
            ) = self.scenarios_town04[np.random.randint(0, len(self.scenarios_town04))]
        else:
            (
                self.spawn_roads,
                self.left_lane_id,
                ...
                self.ego_spawn_idx,
                self.vehicle_init_speed,
            ) = (ROAD_IDs, LEFT_LANE_ID, RIGHT_LANE_ID, EGO_SPAWN_IDX, NPC_INIT_SPEED)
        ...
        waypoints = self.world.get_map().generate_waypoints(
            distance=np.random.randint(
                NPC_SPAWN_POINT_GAP_LOW, NPC_SPAWN_POINT_GAP_HIGH
            )
        )
        road_waypoints = []
        for waypoint in waypoints:
            if (
                waypoint.road_id in self.spawn_roads
                and waypoint.lane_id == self.left_lane_id
            ):
            ...
            batch.append(
                ...
                .then(
                    ApplyVelocity(
                        FutureActor,
                        carla.Vector3D(
                            self.vehicle_init_speed * np.cos(yaw),
                            self.vehicle_init_speed * np.sin(yaw),
                            0,
                        ),
                    )
                )
        ...
        ################# Spawn Ego ##########################
        ego_list = []
        filtered_waypoints = self.carla_handler.filter_waypoints(
            self.carla_handler.get_waypoints(1),
            road_id=self.spawn_roads[0],
            lane_id=self.curr_lane_id,
        )

        spawn_point = filtered_waypoints[
            EGO_SPAWN_IDX
        ].transform  # Select random point from filtered waypoint list #TODO Initialization Scheme Design
        spawn_point.location.z = (
            spawn_point.location.z + 0.1
        )  # To avoid collision during spawn
        vehicle_blueprint = self.carla_handler.blueprint_library.filter(
            EGO_VEHICLE_MAKE
        )[0]
        ...
        ego_list.append(
            SpawnActor(vehicle_blueprint, spawn_point)
            .then(
                ApplyVelocity(
                    FutureActor,
                    carla.Vector3D(
                        self.ego_init_speed * np.cos(yaw),
                        self.ego_init_speed * np.sin(yaw),
                        0,
                    ),
                )
            )
        ...
        # ego_vehicle_wrapped = Vehicle(
        #     self.world,
        #     ego_vehicle_ID,
        # )

        # my_vehicles_wrapped = [
        #     Vehicle(self.world, vehicle.id) for vehicle in my_vehicles
        # ]

        # return ego_vehicle_wrapped, my_vehicles_wrapped, self.target_speed
